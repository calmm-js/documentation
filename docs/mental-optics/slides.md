---
title: Your mental model of optics &mdash; shattered
---
...
Note: Welcome!  The speaker notes in these slides contain some additional notes
...
still left mysterious, please [let me know](https://gitter.im/calmm-js/chat) and
I'll try to add some additional notes.
...
Note: No, that is not enough.  Well, thinking of lenses as paths, for a suitably
...
examples frighten you away from optics.
...
Note: And this presentation isn't any different.
...
Note: Seriously.  Learning optics takes time, but as we'll see they offer a
...
Note: We'll actually not be going too much into theory in the presentation, but
...
desire to keep the presentation shorter.
...
Note: The point of the example is to show that there are indeed many different
ways to view an object using optics and that through each of those views we can
still manipulate the original object.  More on the API later.
...
Note: [See here](https://twitter.com/dan_abramov/status/1046053791899021312).
...
Note: Now that is something that I can hopefully help with.  Optics indeed
provide a different API compared to typical utility libraries.  Let me explain.
...
Note: We will not discuss invariant maintenance in this presentation, but if you
...
based on what seems like the most useful order for partial application.
...
Note: At this point I hope that you realize that this separation of selection
...
using functions such as `map`, `filter`, `reduce`, `chain`, etc... to correctly
and efficiently both select and carry out the desired operation.
...
Note: Traversals are in this sense similar to
[transducers](https://clojure.org/reference/transducers) such that individual
...
Note: It is always nice to receive positive feedback!  However, the phrase
extracted as the title got me thinking.  Why are optics indeed so expressive and
could it be quantified somehow?
...
Note: The number is bogus, of course, because optic compositions can be
...
elimination forms can be combined together.
...
Note: It is harder to think of data manipulation problems where optics could not
...
Note: An ["atom"](https://github.com/calmm-js/karet.util#U-atom) stores an
immutable value.  Lenses can be used to create bidirectional [views of
atoms](https://github.com/calmm-js/karet.util#U-view) such that only a part of
the value stored by a root atom is seen through and can be updated through the
view.  So, if you have an atom that stores a value of type `X` and you have a
lens from type `X` to type `Y`, you can use `view` to create an atom that
seemingly stores a value of type `Y` and treat it just like one.  With
[`mapElems`](https://github.com/calmm-js/karet.util#U-mapElems) one can slice an
atom that contains an array of elements of type `X` into atoms of type `X` and
map those to an array of some type `Y`.
...
Note: Partial Lenses has been designed to support CRUD operations directly,
which makes it straightforward to create CRUD UIs.  The full contacts example
has a filtered and sorted list.  The
[`rewrite`](https://github.com/calmm-js/partial.lenses/#L-rewrite) combinator is
used to keep the contacts in their original order when contacts are written.
The [`reread`](https://github.com/calmm-js/partial.lenses/#L-reread) combinator
...
is generic and knows nothing about history per se.
...
Note: It is also straightforward to create forms with undo-redo and validation.
...
the context version plays the history as an animation.
...
Note: If you look at the documentation of a typical JavaScript UI framework,
you'll no doubt notice a `Counter` component example.  What is different here is
that the trivial looking `Counter` component is not just a toy.  It can be
wired, through lenses, to perform many kinds of state manipulations.  The same
goes for a component like `Checkbox`.  Notice how a similar looking checkbox
input is used in the TodoMVC example to control either an individual `completed`
flag or all of them and both kinds of updates are done using lenses.
...
Note: An assumption here is that the audience is not familiar with traversals so
in the baby steps sandbox the idea is to just show basic traversals like
[`elems`](https://github.com/calmm-js/partial.lenses/#L-elems) and
[`values`](https://github.com/calmm-js/partial.lenses/#L-values) and show how
they can be composed with lenses to get to leaf properties.
...
Note: The gist shows an implementation of a simple UI that only has a single
...
[`L.traverse`](https://github.com/calmm-js/partial.lenses/#L-elems) with the
[`XHR.Succeeded`](https://github.com/calmm-js/karet.xhr/#XHR-Succeeded)
applicative to perform a traversal over a JSON blob to compute a new JSON blob
with additional information obtained using asynchronous XHR operations.
...
Note: As explained in the [implementation
document](https://calmm-js.github.io/partial.lenses/implementation.html), one
could say that most of the Partial Lenses library just gives you combinators for
building traversal functions.  Different operations, like the basic `get` and
`modify` operations and also folds like `sum` and `maximum`, over traversals
just use different applicatives to build the result.
...
Note: Partial Lenses was started during a JavaScript frontend project started in
late 2015.  One thing that has changed since then is that [new projects are
started in
TypeScript](https://twitter.com/VesaKarvonen/status/1052169377263566848).  I
find it unlikely that I would end up working on a JavaScript project where I
might be using Partial Lenses.  While I do think that Partial Lenses is [mostly
typeable](http://ocamllabs.io/higher/lightweight-higher-kinded-polymorphism.pdf)
in TypeScript, it has not been particular designed with TypeScript in mind and I
don't currently have the motivation to develop typings for Partial Lenses.
